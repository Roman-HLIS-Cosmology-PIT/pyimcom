...
class Sca_img:
    ...
    def __init__(self, obsid, scaid, cfg, interpolated=False, add_noise=True, add_objmask=True):
        if interpolated:
            file = fits.open(
                cfg.ds_outpath + "interpolations/" + obsid + "_" + scaid + "_interp.fits", memmap=True
            )
            image_hdu = "PRIMARY"
        else:
            file = fits.open(
    ...
    def apply_noise(self):
        ...
        """
        noiseframe = (
            np.copy(fits.open(self.cfg.ds_indata + self.obsid + "_" + self.scaid + ".fits")["PRIMARY"].data)
            * 1.458
            * 50
    ...
    def make_interpolated(self, ind, scalist, ov_mat, params=None, N_eff_min=0.5):
        ...
        for k, sca_b in enumerate(scalist):
            ...
            if (
                obsid_B != self.obsid and ov_mat[ind, k] >= 0.1
            ):  # Check if this sca_b overlaps sca_a by at least 10%
                N_BinA += 1
        ...
        save_fits(
            ...
            dir=self.cfg.ds_outpath + "interpolations/",
            header=header,
        )
...
def residual_function(
    psi, f_prime, scalist, wcslist, ov_mat, neighbors, thresh, workers, extrareturn=False, ds_model="FR"
):
    """
    Calculate the residual image, = grad(epsilon)
...
def cost_function(p, f, thresh, workers, scalist, ov_mat):
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = [
            executor.submit(cost_function_single, j, sca_a, p, f, scalist, ov_mat, thresh)
...
def linear_search_general(
    ...
    max_resids = residual_function(
        max_psi, f_prime, scalist, wcslist, ov_mat, neighbors, thresh, workers, ds_model=ds_model
    )
    del max_psi
    ...
    min_resids = residual_function(
        min_psi, f_prime, scalist, wcslist, ov_mat, neighbors, thresh, workers, ds_model=ds_model
    )
    del min_psi
    ...
    for k in range(1, n_iter):
        ...
        working_resids = residual_function(
            working_psi, f_prime, scalist, wcslist, ov_mat, neighbors, thresh, workers, ds_model=ds_model
        )
        print(f"Global elapsed t = {(time.time()-t0_global)/60:8.1f}")
...
def linear_search_quadratic(
    p, direction, f, f_prime, grad_current, thresh, workers, scalist, wcslist, ov_mat, ds_model="FR"
):
    """
    ...
    trial_resids = residual_function(
        trial_psi, f_prime, scalist, wcslist, ov_mat, neighbors, thresh, workers, ds_model=ds_model
    )
    ...
    for i in range(i + 1, cfg.cg_maxiter):
        ...
        if grad is None:
            grad = residual_function(
                psi, f_prime, scalist, wcslist, ov_mat, neighbors, thresh, workers, ds_model=cfg.ds_model
            )
            write_to_file(
