...
def run_imsubtract_single(
    ...
    * `display` = any other string : save to ``display+f'_{obsid}_{sca}_{ix:02d}_{iy:02d}.png'``
    ...
    # New memory maps
    H_canvas = np.memmap(
        mmap + "/" + expname[:-5] + "_hcanvas.npy",
        dtype=np.float32,
        mode="w+",
        shape=(A, A),
    )
    KH = np.memmap(
        mmap + "/" + expname[:-5] + "_kh.npy",
        dtype=np.float32,
        mode="w+",
    ...
    # not needed anymore
    del KH
    os.remove(mmap + "/" + expname[:-5] + "_kh.npy")
    del H_canvas
    os.remove(mmap + "/" + expname[:-5] + "_hcanvas.npy")
    ...
    os.remove(mmap + "/" + expname[:-5] + "_data.npy")
