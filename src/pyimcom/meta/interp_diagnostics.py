...
def interp_transfer_function(Rsearch, samp, Cov, Jac=None, epsilon=1.0e-7, umax=1.0, N=128):
    ...
    in_array[0, N_, N_] = 1.0

    in_mask = np.zeros((2 * N_, 2 * N_), dtype=bool)  # trivial mask

    out_array, _, _, _ = MultiInterp(
        in_array,
        in_mask,
        (N, N),
        np.array([N_, N_]) - N / 4.0 / umax * Jac @ np.array([1, 1]),
        Jac / (2 * umax),
        Rsearch,
        samp,
        Cov,
    ...
    out_array_ft = np.fft.fft2(np.fft.fftshift(out_array)).real / (2.0 * umax) ** 2  # normalized FT
    return np.fft.fftshift(out_array_ft)


# def _sample():
#     t = interp_transfer_function(6.0, 4.5, np.array([0.64, 0., 0.32]), Jac = np.array([[0.5,0.],[0.,0.5]]),
#             epsilon=1.0e-7, umax=1.0, N=256)
